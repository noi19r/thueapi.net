...
const initTabulator = () => {
  tabulator.value = new Tabulator(tableRef.value, {
    ajaxURL: `/api/bank/zalopay/transaction/${router.currentRoute.value.params.id}`,
    ajaxConfig: {
      headers: {
        ...
        formatter(cell) {
          return `<div>
                <div class="font-medium whitespace-nowrap">${
                  cell.getData().info[1] ? cell.getData().info[1].value : cell.getData().info[0].name
                }</div>
                <div class="text-slate-500 text-xs whitespace-nowrap">${cell.getData().info[0].value}</div>
              </div>`
        }
      },
